...
function themememe_content_nav( $nav_id ) {
	...
	<?php if ( is_single() ) : ?>
		...
		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-chevron-left"></i> Article précédent', 'Article précédent', 'themememe' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '<span class="meta-nav">' . _x( 'Article suivant <i class="fa fa-chevron-right"></i>', 'Article suivant', 'themememe' ) . '</span> %title' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav"><i class="fa fa-chevron-left"></i></span> Article précédent', 'themememe' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Article suivant <span class="meta-nav"><i class="fa fa-chevron-right"></i></span>', 'themememe' ) ); ?></div>
		<?php endif; ?>
...
function themememe_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'themememe' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Modifier', 'themememe' ), '<span class="comment-meta"><span class="edit-link">', '</span></span>' ); ?>
		</div>
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			...
			<p class="comment-awaiting-moderation"><?php _e( 'Votre commentaire est en attente de modération', 'themememe' ); ?></p>
			<?php endif; ?>
			...
			<div class="comment-meta comment-footer">
				<?php edit_comment_link( __( 'Modifier', 'themememe' ), '<span class="edit-link">', '</span>' ); ?>
				<?php
...
function themememe_posted_on() {
	...
	printf( __( '<span class="posted-on">%1$s</span>', 'themememe' ),
...
if ( ! function_exists( 'themememe_posted_by' ) ) :
/**
 * Prints HTML with meta information for the current post author.
 */
function themememe_posted_by() {
	printf( __( '<span class="byline">%1$s</span>', 'themememe' ),
